...
public class studentProfileActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private DatabaseReference mdatabaseRef, mRoofRef, firstNameRef, lastNameRef, headlineRef, fullNameRef, tFirstNameRef, tLastNameRef, tFullNameRef;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tFirstNameRef = mdatabaseRef.child("tutors").child(userInfo.getId()).child("firstName");
        tLastNameRef = mdatabaseRef.child("tutors").child(userInfo.getId()).child("lastName");
        tFullNameRef =  mdatabaseRef.child("tutors").child(userInfo.getId()).child("fullName");
        ...
        //Determine whether or not user is a tutor/ has a tutor profile
        DatabaseReference isTutorRef = mdatabaseRef.child("tutors").child(userInfo.getId());
        isTutorRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
    ...
    public void onClick(View v) {
        if(v == editButton) {
            if (editing) {
                editing = false;
                if(isTutor){
                    tFirstNameRef.setValue(firstName.getText().toString());
                    tLastNameRef.setValue(lastName.getText().toString());
                    tFullNameRef.setValue(firstName.getText().toString().trim()+" " +lastName.getText().toString().trim());
                }
